...
uninstall_trusttunnel_action() {
  ...
  if [[ "$confirm" =~ ^[Yy]$ ]]; then
    ...
    # Remove hysteria2 folder if exists
    if [ -d "/hysteria2" ]; then
      echo "🗑️ Removing 'hysteria2' folder..."
      sudo rm -rf /hysteria2
      print_success "'hysteria2' folder removed successfully."
    else
      echo "⚠️ 'hysteria2' folder not found."
    fi
...
add_new_direct_server_action() {
  ...
  while true; do
    ...
    read -p "" listen_port_input
    listen_port=${listen_port_input:-8800} # Apply default if empty
    if validate_port "$listen_port"; then
      break
    else
      print_error "Invalid port number. Please enter a number between 1 and 65535." # Invalid port number. Please enter a number between 1 and 65535.
  ...
  if systemctl is-active --quiet trusttunnel-direct.service || systemctl is-enabled --quiet trusttunnel-direct.service; then
    echo -e "${YELLOW}🛑 Stopping existing Direct Trusttunnel service...${RESET}" # Stopping existing Direct Trusttunnel service...
    sudo systemctl stop trusttunnel-direct.service > /dev/null 2>&1
    echo -e "${YELLOW}🗑️ Disabling and removing existing Direct Trusttunnel service...${RESET}" # Disabling and removing existing Direct Trusttunnel service...
    ...
    sudo systemctl daemon-reload > /dev/null 2>&1
    print_success "Existing Direct TrustTunnel service removed." # Direct TrustTunnel service removed.
  fi
...
EOF
  ...
  echo -e "${CYAN}🚀 Enabling and starting Direct Trusttunnel service...${RESET}" # Enabling and starting Direct Trusttunnel service...
  ...
  print_success "Direct TrustTunnel service started successfully!" # Direct TrustTunnel service started successfully!
  echo ""
  echo -e "${YELLOW}Press Enter to return to previous menu...${RESET}" # Press Enter to return to previous menu...
  read -p ""
}
...
while true; do
  ...
  echo -e "${MAGENTA}1) Install Hysteria2${RESET}" # Install Hysteria2
  echo -e "${MAGENTA}2) Install Rstun${RESET}" # Install TrustTunnel
  echo -e "${CYAN}3) Rstun reverse tunnel${RESET}" # Rstun reverse tunnel
  echo -e "${CYAN}4) Rstun direct tunnel${RESET}" # Rstun direct tunnel
  ...
  case $choice in
    ...
    3)
      ...
      read -p "" tunnel_choice # Your choice:
